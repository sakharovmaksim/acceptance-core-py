...
def get_images_diff_percent_and_save_diff_img(
    reference_model_local_path: Path,
    candidate_model_local_path: Path,
    diff_image_to_save_local_path: Path,
) -> float:
    """Return rounded diff percent from visual models and save diff image with inverted colors to local Path"""
    diff_percent = diff(
        str(reference_model_local_path.absolute()),
    ...
    logging.info(
        f"Inverting Diff-image to {diff_image_to_save_local_path.absolute()}. Image mode {image.mode=}"
    )
