...
class ClickHouseDbClient:
    ...
    def get_db_client(self, db: str = "test") -> Client:
        ...
        self.__db_client = clickhouse_driver.Client(
            host=self.__clickhouse_host, port=self.__clickhouse_port, database=db
        )
        return self.__db_client

    def select_from_table(
        self, table: str, search_condition: str = None, limit: int = 30
    ) -> List:
    ...
    def select_first_from_table(
        self, table: str, search_condition: str = None
    ) -> Tuple:
        result = self.select_from_table(
            table=table, search_condition=search_condition, limit=1
        )
        if len(result) == 0:
            return tuple()
        return_result = result[0]
        logging.info(
            f"Returning first tuple from list-result. Tuple is: '{return_result}'"
        )
