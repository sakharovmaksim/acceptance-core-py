import logging
from prometheus_client import CollectorRegistry, push_to_gateway, Info, delete_from_gateway
...
class PrometheusClient:
    ...
    def push_test_failed_metric_to_gateway(self) -> bool:
        ...
        test_locator = env.get_test_file_name().replace('/', '.') + '.' + env.get_test_name()
        ...
        i = Info(name='acceptance_tests_failures',
        ...
        try:
            push_to_gateway(gateway=self.__gateway_url, job=test_locator + '_' + date, registry=registry)
        except Exception as exception:
            logging.error(f"Error in sending test metrics to Prometheus: {str(exception)}")
            return False
