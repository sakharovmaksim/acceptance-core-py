...
from acceptance_core_py.helpers.clients.webdav_screenshot_storage_data import WebDavScreenshotStorageData
...
class WebDavClient:
    ...
    def create_webdav_screenshot_storage_data(self,
                                              ...
                                              ScreenshotLocalStorageData) -> WebDavScreenshotStorageData:
        ...
        if gitlab_branch_name:
            webdav_dir_for_upload = self.__screenshots_base_dir + '/' + gitlab_branch_name + '/' + remote_dir_name
        else:
    ...
    def publish_screenshot(self, screenshot_local_storage_data: ScreenshotLocalStorageData) -> Optional[str]:
        ...
        webdav_screenshot_storage_data = self.create_webdav_screenshot_storage_data(screenshot_local_storage_data)
        webdav_dir_for_upload: str = webdav_screenshot_storage_data.screenshots_dir_name
        webdav_file_uploaded_url: str = webdav_screenshot_storage_data.screenshot_url_without_prefix
        ...
        if not self.__client.check(webdav_dir_for_upload):
            logging.debug(f"Creating WebDav-dir for screenshot: '{webdav_dir_for_upload=}'")
            ...
            if len(dir_list) == 2:
                ...
                raise ATException(f"{dir_list=} not supported for creating dirs in WebDav")

        try:
            self.__client.upload_sync(webdav_file_uploaded_url, screenshot_local_path)
        except Exception:
            logging.warning(f"Could not upload screenshot from '{screenshot_local_path=}' "
    ...
    def download_file(self, from_url: str, screenshot_local_storage_data: ScreenshotLocalStorageData) -> bool:
        ...
        logging.info(f"Downloading file from WebDav. From '{from_url=}', saving to '{full_local_file_path=}'")

        pathlib.Path(local_dir_path).mkdir(parents=True, exist_ok=True)
        img_data = requests.get(from_url, auth=(webdav_keys.webdav_login, webdav_keys.webdav_password)).content

        min_file_size_bytes = 300
        if img_data.__sizeof__() < min_file_size_bytes:
            logging.info(f"File by {from_url=} is less than {min_file_size_bytes=}. "
