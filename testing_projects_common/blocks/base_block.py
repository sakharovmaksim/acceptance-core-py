...
class BaseBlock(ABC):
    """Базовый блок. От него должны наследоваться все блоки в проекте"""
    ...
    custom_wait_timeout_in_sec: int | None = None
    ...
    def get_block_selector_from_context_selector(
        ...
        if self.block_class:
            ...
            return context_selector.child_by_attribute(
                self.block_attribute_name, self.block_attribute_value
            )
        else:
            logging.debug(
                f"Block do not have your own content selector, "
                f"return 'context_selector': '{context_selector}'"
            )
            return context_selector

    def wait_for_ready(self) -> BaseBlock:
        waiting_actions.wait_for_element_visible(
            self.me,
            f"Could not waiting for visibility {self.__class__.__name__}",
            self.custom_wait_timeout_in_sec,
        )
        return self
