...
TAG="sakharovmaksim/service-integration-tests-base-image-python:latest"
...
docker exec $CONTAINER_NAME rsync -a /local_project/. /tests-core-dir/ --exclude venv --exclude output --exclude tmp --exclude .git
...
docker exec $CONTAINER_NAME pytest /tests-core-dir/ -s --html=/tests-core-dir/output/report.html --self-contained-html \
    -c /tests-core-dir/$1

# Копируем репорты от тестов обратно в --volume-папку local_project
docker exec $CONTAINER_NAME rsync -a /tests-core-dir/output/ /local_project/output/
